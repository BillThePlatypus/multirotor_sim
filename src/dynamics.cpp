...
// u = [F(N), Taux(N-m), Tauy, Tauz]
    void Dynamics::f(const State &x, const Vector4d &ft, ErrorState &dx) const
    {
        ...
        dx.q = x.w;
        dx.w = inertia_inv_ *
               (ft.segment<3>(TAUX) - x.w.cross(inertia_matrix_ * x.w) - angular_drag_ * x.w.cwiseProduct(x.w));
    }

    void Dynamics::f(const State &x, const Vector4d &u, ErrorState &dx, Vector6d &imu) const
    {
        f(x, u, dx);
        imu.segment<3>(ACC) = q_b2u_.rotp(
                dx.v + x.w.cross(x.v) + x.w.cross(x.w.cross(p_b2u_)) + dx.w.cross(p_b2u_) - x.q.rotp(gravity_));
