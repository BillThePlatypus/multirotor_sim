...
namespace multirotor_sim
{
    ...
    void ReferenceController::computeControl(const double &t, const State &x, const State &x_c, const Vector4d &ur,
                                             Vector4d &u)
    {
        // Function constants
        ...
        double sh_inv_hat_dot =
                -sh_ks_ * multirotor_sim::G * s_prev_ * (xhat_.v - vhat_).transpose() * R_v1_to_b.transpose() * e3;
        vhat_ += vhat_dot * dt;
    ...
    void ReferenceController::load(const std::string filename)
    {
        if (file_exists(filename))
        {
            ...
            if (control_type_ == 0)
            {
                ...
                nlc_.init(K_p_, K_v_, K_d_, path_type_, max_, traj_heading_walk_, traj_heading_straight_gain_, rng_,
                          udist_);
            } else if (control_type_ == 1)
            {
                // Load LQR parameters
