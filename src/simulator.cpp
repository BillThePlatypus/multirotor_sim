...
namespace multirotor_sim
{
    ...
    void Simulator::update_camera_meas()
    {
        ...
        if (std::round((t_ - last_camera_update_) * t_round_off_) / t_round_off_ >= 1.0 / camera_update_rate_)
        {
            ...
            double pub_time =
                    t_ + std::max(camera_transmission_time_ + normal_(rng_) * camera_transmission_noise_, 0.0);
    ...
    void Simulator::update_vo_meas()
    {
        ...
        if (delta.segment<3>(0).norm() >= vo_delta_position_ || delta.segment<3>(3).norm() >= vo_delta_attitude_)
        {
            ...
            Xformd T_c2ck;
            T_c2ck.t_ = transform_body_to_camera_.rotp(
                    T_i2b.q().rotp(X_I2bk_.t() + X_I2bk_.q().inverse().rotp(transform_body_to_camera_.t()) -
                                   (T_i2b.t() + T_i2b.q().inverse().rotp(transform_body_to_camera_.t()))));
            T_c2ck.q_ = transform_body_to_camera_.q_.inverse() * T_i2b.q().inverse() * X_I2bk_.q().inverse() *
                        transform_body_to_camera_.q_;
        ...
        if (std::round(dt * t_round_off_) / t_round_off_ >= 1.0 / gnss_update_rate_)
        {
            ...
            for (i = 0, sat = satellites_.begin(); sat != satellites_.end(); sat++, i++)
            {
                ...
                if (multipath_area_.inside(p_ned))
                {
                    ...
                    {
                        if (uniform_(rng_) <
                            (multipath_prob_ + (0.7 * (1.0 - multipath_prob_))) * 1.0 / gnss_update_rate_)
                        {
