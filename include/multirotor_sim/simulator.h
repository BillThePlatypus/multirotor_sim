...
namespace multirotor_sim
{
    ...
    public:
        ...
        const Vector6d &imu() const
        { return dyn_.imu_; }

        const State &state() const
        { return dyn_.get_state(); }

        State &state()
        { return dyn_.get_state(); }

        const Vector4d &input() const
        { return u_; }

        Vector4d &input()
        { return u_; }

        const State &commanded_state() const
        { return xc_; }

        Vector4d &reference_input()
        { return ur_; }

        const Vector4d &reference_input() const
        { return ur_; }
