...
bool get_yaml_eigen(const std::string key, const std::string filename, Eigen::MatrixBase<Derived1> &val,
    ...
    if (node[key])
    {
        ...
        } else
        {
            throw std::runtime_error("Eigen Matrix Size does not match parameter size for " + key + " in " + filename +
                                     ". Requested " + std::to_string(Derived1::RowsAtCompileTime) + "x" +
                                     std::to_string(Derived1::ColsAtCompileTime) +
...
bool get_yaml_diag(const std::string key, const std::string filename, Eigen::MatrixBase<Derived> &val,
                   bool print_error = true)
{
...
template<typename Derived1>
bool get_yaml_priority_eigen(const std::string key, const std::string file1, const std::string file2,
...
template<typename Derived>
Derived
randomNormal(double stdev, std::normal_distribution<typename Derived::Scalar> &dist, std::default_random_engine &gen)
{
    Derived vec;
